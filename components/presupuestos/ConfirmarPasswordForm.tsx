...
import { eliminarPresupuesto } from "@/actions/eliminar-presupuesto-action";
import MensajeError from "../ui/MensajeError";
import { useEffect } from "react";
import { toast } from "react-toastify";

export default function ConfirmarPasswordForm() {
  const pathname = usePathname();
  const router = useRouter();
  const searchParams = useSearchParams();
  const idPresupuesto = +searchParams.get("idEliminarPresupuesto")!;

  const closeModal = () => {
    const hideModal = new URLSearchParams(searchParams.toString());
    hideModal.delete("idEliminarPresupuesto");
    router.replace(`${pathname}?${hideModal}`);
  };

  const eliminarPresupuestoPassword = eliminarPresupuesto.bind(
    null,
    idPresupuesto
  );

  const [state, dispatch] = useFormState(eliminarPresupuestoPassword, {
    errores: [],
    success: "",
  });
